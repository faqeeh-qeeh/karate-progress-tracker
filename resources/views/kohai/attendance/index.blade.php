    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <span id="scan-status-badge" class="px-3 py-1 bg-amber-50 text-amber-700 border border-amber-200 rounded-full text-xs font-bold">
                    Siap Memindai
                </span>
            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500 font-medium bg-slate-50 p-3.5 rounded-2xl border border-slate-200/80">
                <p class="text-slate-600">📍 Jika kamera live disekat HTTP, gunakan tombol <b>Foto QR Kamera HP</b>.</p>
                <div class="flex flex-wrap gap-2">
                    ...
                    <button type="button" onclick="document.getElementById('qr-file-input').click()" class="px-3.5 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold rounded-xl text-xs hover:brightness-110 transition shadow-xs flex items-center gap-1">
                        <span>📸</span> Ambil Foto QR (Kamera HP)
                    </button>
                    <button type="button" id="btn-start-camera" class="px-3.5 py-1.5 bg-brand-secondary text-slate-900 font-bold rounded-xl text-xs hover:brightness-105 transition shadow-xs">
                        Aktifkan Live Stream
                    </button>
                    <button type="button" id="btn-stop-camera" class="px-3.5 py-1.5 bg-slate-200 text-slate-700 font-bold rounded-xl text-xs hover:bg-slate-300 transition hidden">
                        Matikan Stream
                    </button>
                </div>
            </div>
        ...
        <div class="lg:col-span-1 bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm flex flex-col justify-between space-y-4">
            <div class="space-y-3">
                <div class="border-b border-slate-100 pb-3">
                    <h3 class="text-base font-extrabold text-slate-900 flex items-center gap-2">
                        <span>⌨️</span> Input Kode Manual
                    </h3>
                    <p class="text-xs text-slate-500 mt-0.5">Gunakan form ini jika kamera tidak bisa diakses.</p>
                </div>

                <form id="scan-form" action="{{ route('kohai.attendance.scan') }}" method="POST" class="space-y-4">
                    @csrf
                    <div class="space-y-1.5">
                        <label for="qr_token" class="block text-xs font-bold text-slate-700">Token Absensi / Kode QR</label>
                        <input type="text" id="qr_token" name="qr_token" required placeholder="Tempel / ketik token QR di sini" class="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-300 focus:ring-2 focus:ring-brand-primary focus:border-brand-primary outline-none transition font-mono">
                    </div>

                    <button type="submit" class="w-full py-3 bg-brand-primary hover:bg-brand-primary/90 text-white rounded-xl font-bold text-xs shadow-md transition flex items-center justify-center gap-2">
            ...
            <div class="bg-blue-50 p-4 rounded-2xl border border-blue-200/80 text-xs text-blue-900 space-y-1 font-medium">
                <p class="font-bold flex items-center gap-1 text-brand-primary">💡 Catatan Presensi:</p>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5">Catatan sesi latihan yang telah Anda ikuti.</p>
            </div>
        </div>
            ...
            @forelse($myAttendances as $att)
                <div class="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm hover:shadow-md transition-all space-y-3">
                    ...
                    <div class="flex items-center justify-between border-b border-slate-100 pb-2.5">
                        ...
                        <span class="px-3 py-0.5 text-xs font-extrabold rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
                            {{ $att->status }}
                        </span>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="flex flex-col gap-1 text-xs text-slate-500 font-medium pt-1 border-t border-slate-100">
                            <p class="flex items-center justify-between">
                                <span>Penanggungjawab:</span>
                                <strong class="text-brand-primary">Senpai {{ $att->session->senpai->name ?? '-' }}</strong>
                            </p>
                            <p class="flex items-center justify-between font-mono text-[11px] text-slate-600">
                    ...
                    <tbody class="divide-y divide-slate-100 whitespace-nowrap font-medium">
                        @forelse($myAttendances as $att)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="py-3.5 px-4 font-bold text-slate-900">{{ $att->session->title ?? 'Sesi Latihan' }}</td>
                                <td class="py-3.5 px-4 text-xs font-bold text-brand-primary">
