...
@section('title', 'Pengaturan & Tampilan Kohai')
...
<div class="space-y-6 max-w-6xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm transition-colors duration-200">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Pengaturan & Tampilan Kohai</h1>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Konfigurasi preferensi tema antarmuka (Dark / Light Mode) dan informasi akun keanggotaan</p>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden transition-colors duration-200">
        <div class="p-6 border-b border-slate-100 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Pilih gaya visual yang paling nyaman untuk Anda saat mengakses portal Kohai</p>
            </div>

            <!-- Current Active Theme Badge -->
            <div id="kohai-theme-status-badge" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-brand-primary/10 dark:bg-brand-primary/20 text-brand-primary dark:text-brand-secondary text-xs font-bold self-start sm:self-auto">
                <span id="kohai-theme-badge-icon">🌙</span>
                <span id="kohai-theme-badge-text">Mode Gelap Aktif</span>
            </div>
        </div>
        ...
        <div class="p-6 sm:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <button type="button" onclick="setKohaiTheme('light')" id="theme-card-light" class="relative text-left p-5 rounded-2xl border-2 transition-all duration-200 cursor-pointer group hover:shadow-md border-slate-200 hover:border-brand-primary dark:border-slate-800 dark:hover:border-slate-600 bg-slate-50/50 dark:bg-slate-950/40">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Tampilan cerah dengan kontras bersih, cocok untuk pemakaian di tempat terang.</p>
                </button>

                <!-- Dark Mode Card Option -->
                <button type="button" onclick="setKohaiTheme('dark')" id="theme-card-dark" class="relative text-left p-5 rounded-2xl border-2 transition-all duration-200 cursor-pointer group hover:shadow-md border-slate-200 hover:border-brand-primary dark:border-slate-800 dark:hover:border-slate-600 bg-slate-50/50 dark:bg-slate-950/40">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Mengurangi ketegangan mata di ruangan redup dan hemat daya layar smartphone/laptop.</p>
                </button>

                <!-- System Auto Preference Option -->
                <button type="button" onclick="setKohaiTheme('system')" id="theme-card-system" class="relative text-left p-5 rounded-2xl border-2 transition-all duration-200 cursor-pointer group hover:shadow-md border-slate-200 hover:border-brand-primary dark:border-slate-800 dark:hover:border-slate-600 bg-slate-50/50 dark:bg-slate-950/40">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Menyesuaikan tema secara dinamis dengan setelan tema sistem perangkat Anda.</p>
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-5 transition-colors duration-200">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-4">
            <h3 class="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                ...
                <span>Informasi Akun & Keanggotaan Kohai</span>
            </h3>
            <a href="{{ route('kohai.profile.index') }}" class="text-xs font-bold text-brand-primary dark:text-brand-secondary hover:underline flex items-center gap-1">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Nama Anggota</span>
            ...
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                ...
                @if($user->kohaiProfile?->rank)
                    <span class="inline-flex items-center gap-1.5 text-sm font-bold text-slate-800 dark:text-slate-200">
                        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background-color: {{ $user->kohaiProfile->rank->belt->color_code ?? '#eab308' }}"></span>
                        <span>{{ $user->kohaiProfile->rank->belt->name ?? 'Sabuk' }} - {{ $user->kohaiProfile->rank->name }}</span>
                    </span>
                @else
            ...
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Kategori Keanggotaan</span>
                <span class="inline-block px-2.5 py-0.5 rounded-md font-bold text-xs uppercase {{ $user->kohaiProfile?->type === 'polindra' ? 'bg-blue-100 text-blue-800 dark:bg-blue-950/60 dark:text-blue-300' : 'bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300' }}">
                    {{ $user->kohaiProfile?->type === 'polindra' ? 'Internal Polindra' : 'Luar Polindra' }}
                </span>
            </div>
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Prodi / Instansi</span>
                <span class="text-sm font-bold text-slate-800 dark:text-slate-200 truncate block">
                    {{ $user->kohaiProfile?->type === 'polindra' ? ($user->kohaiProfile->studyProgram->name ?? '-') : ($user->kohaiProfile->institution ?? '-') }}
                </span>
            </div>
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
...
<script>
    function updateKohaiSettingsPageUI() {
        ...
        // Status badge
        const badgeIcon = document.getElementById('kohai-theme-badge-icon');
        const badgeText = document.getElementById('kohai-theme-badge-text');
    ...
    function setKohaiTheme(theme) {
        ...
        updateKohaiSettingsPageUI();
        window.dispatchEvent(new CustomEvent('karateThemeChanged', { 
            detail: { isDark: document.documentElement.classList.contains('dark') } 
        }));
    }

    window.addEventListener('karateThemeChanged', updateKohaiSettingsPageUI);
    document.addEventListener('DOMContentLoaded', updateKohaiSettingsPageUI);
</script>
@endsection
