        ...
        @forelse($users as $u)
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-sm hover:shadow-md transition-all space-y-3.5">
                ...
                <div class="pt-2 border-t border-slate-100 flex items-center gap-2">
                    ...
                    <!-- Delete Button -->
                    @if($u->id !== auth()->id())
                        <form action="{{ route('admin.users.destroy', $u->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun {{ $u->name }}? Tindakan ini tidak dapat dibatalkan.');" class="flex-1">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full py-2.5 px-3 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200/80 text-xs font-bold rounded-xl transition-all shadow-xs flex items-center justify-center gap-1.5">
                    ...
                    @else
                        <div class="flex-1 py-2.5 px-3 bg-slate-100 text-slate-400 border border-slate-200 text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 cursor-not-allowed select-none" title="Tidak dapat menghapus akun sendiri">
