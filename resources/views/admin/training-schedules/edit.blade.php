...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Edit Jadwal Latihan</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">Perbarui informasi jadwal latihan rutin mingguan atau latihan tambahan</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm transition-colors">
        <form action="{{ route('admin.training-schedules.update', $trainingSchedule->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label class="relative flex items-center gap-3 p-4 rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-brand-primary dark:hover:border-brand-primary bg-slate-50/50 dark:bg-slate-800/80 cursor-pointer transition has-[:checked]:border-brand-primary has-[:checked]:bg-blue-50/40 dark:has-[:checked]:bg-blue-950/30">
                        <input type="radio" name="type" value="rutin" {{ old('type', $trainingSchedule->type) === 'rutin' ? 'checked' : '' }} class="w-4 h-4 text-brand-primary focus:ring-brand-primary" onchange="toggleType(this.value)">
                        <div>
                    ...
                    <label class="relative flex items-center gap-3 p-4 rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-brand-primary dark:hover:border-brand-primary bg-slate-50/50 dark:bg-slate-800/80 cursor-pointer transition has-[:checked]:border-brand-primary has-[:checked]:bg-blue-50/40 dark:has-[:checked]:bg-blue-950/30">
                        <input type="radio" name="type" value="tambahan" {{ old('type', $trainingSchedule->type) === 'tambahan' ? 'checked' : '' }} class="w-4 h-4 text-brand-primary focus:ring-brand-primary" onchange="toggleType(this.value)">
                        <div>
            ...
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $trainingSchedule->title) }}" required
            ...
            @php
                $currentDays = old('days_of_week', $trainingSchedule->days_of_week ?? []);
            @endphp
            <div id="section-days" class="{{ old('type', $trainingSchedule->type) === 'tambahan' ? 'hidden' : '' }} space-y-2">
                    ...
                    @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $hari)
                        <label class="cursor-pointer">
                            <input type="checkbox" name="days_of_week[]" value="{{ $hari }}"
                                {{ in_array($hari, $currentDays) ? 'checked' : '' }}
                                class="peer sr-only">
            ...
            <div id="section-specific-date" class="{{ old('type', $trainingSchedule->type) === 'tambahan' ? '' : 'hidden' }} space-y-1.5">
                ...
                <input type="date" name="specific_date" id="specific_date" value="{{ old('specific_date', $trainingSchedule->specific_date ? $trainingSchedule->specific_date->format('Y-m-d') : '') }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $trainingSchedule->start_date ? $trainingSchedule->start_date->format('Y-m-d') : '') }}" required
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $trainingSchedule->end_date ? $trainingSchedule->end_date->format('Y-m-d') : '') }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" value="{{ old('start_time', substr($trainingSchedule->start_time, 0, 5)) }}" required
                ...
                <div>
                    ...
                    <input type="time" name="end_time" id="end_time" value="{{ old('end_time', $trainingSchedule->end_time ? substr($trainingSchedule->end_time, 0, 5) : '') }}"
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <label class="relative flex items-center gap-3 p-3.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-brand-primary dark:hover:border-brand-primary bg-slate-50/50 dark:bg-slate-800/80 cursor-pointer transition has-[:checked]:border-brand-primary has-[:checked]:bg-blue-50/40 dark:has-[:checked]:bg-blue-950/30">
                            <input type="radio" name="location_type" value="polindra" {{ old('location_type', $trainingSchedule->location_type) === 'polindra' ? 'checked' : '' }} class="w-4 h-4 text-brand-primary focus:ring-brand-primary">
                            <div>
                        ...
                        <label class="relative flex items-center gap-3 p-3.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-brand-primary dark:hover:border-brand-primary bg-slate-50/50 dark:bg-slate-800/80 cursor-pointer transition has-[:checked]:border-brand-primary has-[:checked]:bg-blue-50/40 dark:has-[:checked]:bg-blue-950/30">
                            <input type="radio" name="location_type" value="luar_polindra" {{ old('location_type', $trainingSchedule->location_type) === 'luar_polindra' ? 'checked' : '' }} class="w-4 h-4 text-brand-primary focus:ring-brand-primary">
                            <div>
                ...
                <div>
                    ...
                    <input type="text" name="location_detail" id="location_detail" value="{{ old('location_detail', $trainingSchedule->location_detail) }}"
                ...
                <div>
                    ...
                    <input type="url" name="maps_url" id="maps_url" value="{{ old('maps_url', $trainingSchedule->maps_url) }}"
            ...
            <div class="p-4 sm:p-5 rounded-2xl bg-slate-50/80 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700 space-y-4">
                <div class="flex items-center justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="email_reminder" value="1" id="toggle-reminder" {{ old('email_reminder', $trainingSchedule->email_reminder ? '1' : '0') == '1' ? 'checked' : '' }} class="sr-only peer" onchange="toggleReminderTime(this.checked)">
                        <div class="w-11 h-6 bg-slate-200 dark:bg-slate-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-brand-primary"></div>
                    </label>
                </div>

                <div id="section-reminder-time" class="{{ old('email_reminder', $trainingSchedule->email_reminder ? '1' : '0') == '1' ? '' : 'hidden' }} pt-3 border-t border-slate-200/80 dark:border-slate-700">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 cursor-pointer hover:border-brand-primary dark:hover:border-brand-primary transition">
                            <input type="radio" name="reminder_time" value="pagi" {{ old('reminder_time', $trainingSchedule->reminder_time ?? 'pagi') === 'pagi' ? 'checked' : '' }} class="w-4 h-4 text-brand-primary focus:ring-brand-primary">
                            <div>
                        ...
                        <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 cursor-pointer hover:border-brand-primary dark:hover:border-brand-primary transition">
                            <input type="radio" name="reminder_time" value="malam" {{ old('reminder_time', $trainingSchedule->reminder_time) === 'malam' ? 'checked' : '' }} class="w-4 h-4 text-brand-primary focus:ring-brand-primary">
                            <div>
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3"
                    ...
                    class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/80 text-slate-900 dark:text-white text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary focus:bg-white dark:focus:bg-slate-800 transition placeholder:text-slate-400 dark:placeholder:text-slate-500">{{ old('notes', $trainingSchedule->notes) }}</textarea>
            </div>
            ...
            <div class="flex items-center gap-3 p-3.5 rounded-xl bg-slate-50/80 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700">
                <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $trainingSchedule->is_active ? '1' : '0') == '1' ? 'checked' : '' }}
