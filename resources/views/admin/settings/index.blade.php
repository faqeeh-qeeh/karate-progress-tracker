...
@section('title', 'Pengaturan Sistem & Tampilan')
...
<div class="space-y-6 max-w-6xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm transition-colors duration-200">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Pengaturan Sistem & Tampilan</h1>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Konfigurasi preferensi tema antarmuka (Dark / Light Mode) dan informasi workspace admin</p>
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm overflow-hidden transition-colors duration-200">
        <div class="p-6 border-b border-slate-100 dark:border-slate-800/80 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Pilih gaya visual yang paling nyaman untuk Anda bekerja dalam mengelola data karate</p>
            </div>

            <!-- Current Active Theme Badge -->
            <div id="settings-theme-status-badge" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-brand-primary/10 dark:bg-brand-primary/20 text-brand-primary dark:text-brand-secondary text-xs font-bold self-start sm:self-auto">
                <span id="settings-theme-badge-icon">🌙</span>
                <span id="settings-theme-badge-text">Mode Gelap Aktif</span>
            </div>
        </div>
        ...
        <div class="p-6 sm:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <button type="button" onclick="setAdminTheme('light')" id="theme-card-light" class="relative text-left p-5 rounded-2xl border-2 transition-all duration-200 cursor-pointer group hover:shadow-md border-slate-200 hover:border-brand-primary dark:border-slate-800 dark:hover:border-slate-600 bg-slate-50/50 dark:bg-slate-950/40">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Tampilan cerah dengan kontras bersih, cocok untuk ruangan terang.</p>
                </button>

                <!-- Dark Mode Card Option -->
                <button type="button" onclick="setAdminTheme('dark')" id="theme-card-dark" class="relative text-left p-5 rounded-2xl border-2 transition-all duration-200 cursor-pointer group hover:shadow-md border-slate-200 hover:border-brand-primary dark:border-slate-800 dark:hover:border-slate-600 bg-slate-50/50 dark:bg-slate-950/40">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Mengurangi ketegangan mata di ruangan redup dan hemat konsumsi daya layar.</p>
                </button>

                <!-- System Auto Preference Option -->
                <button type="button" onclick="setAdminTheme('system')" id="theme-card-system" class="relative text-left p-5 rounded-2xl border-2 transition-all duration-200 cursor-pointer group hover:shadow-md border-slate-200 hover:border-brand-primary dark:border-slate-800 dark:hover:border-slate-600 bg-slate-50/50 dark:bg-slate-950/40">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Menyesuaikan tema secara dinamis dengan pengaturan OS / browser Anda.</p>
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-5 transition-colors duration-200">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Nama Aplikasi</span>
                <span class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ $systemInfo['app_name'] }}</span>
            </div>
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Framework Version</span>
                <span class="text-sm font-mono font-bold text-slate-800 dark:text-slate-200">Laravel v{{ $systemInfo['laravel_version'] }}</span>
            </div>
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">PHP Environment</span>
                <span class="text-sm font-mono font-bold text-slate-800 dark:text-slate-200">PHP {{ $systemInfo['php_version'] }}</span>
            </div>
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
            ...
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Mode Lingkungan</span>
                <span class="inline-block px-2.5 py-1 rounded-md font-bold text-xs uppercase {{ $systemInfo['app_env'] === 'production' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-400' : 'bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-400' }}">
                    {{ $systemInfo['app_env'] }}
                </span>
            </div>
            <div class="p-4 rounded-xl bg-slate-50/70 dark:bg-slate-950/50 border border-slate-200/60 dark:border-slate-800">
                <span class="block text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Versi Antarmuka</span>
                <span class="text-sm font-bold text-brand-primary">v1.0.0 (Polindra UI)</span>
            </div>
        </div>
    </div>
</div>
...
<script>
    function updateSettingsPageUI() {
        ...
        // Status badge
        const badgeIcon = document.getElementById('settings-theme-badge-icon');
        const badgeText = document.getElementById('settings-theme-badge-text');
    ...
    window.addEventListener('karateThemeChanged', updateSettingsPageUI);
    document.addEventListener('DOMContentLoaded', updateSettingsPageUI);
</script>
@endsection
