...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">Edit Master Sabuk</h1>
            <p class="text-xs text-slate-500 mt-1 font-medium">Perbarui nama, warna, atau keterangan filosofis sabuk</p>
        </div>
        ...
        <form action="{{ route('admin.belts.update', $belt->id) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="color" id="color-picker" value="{{ old('color_code', $belt->color_code ?? '#3B82F6') }}" onchange="document.getElementById('color_code').value = this.value" class="w-12 h-11 p-1 rounded-xl border border-slate-300 cursor-pointer bg-white">
            ...
            <div>
                ...
                <div class="relative rounded-xl">
                    <textarea name="description" id="description" rows="3"
                        ...
                        class="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50/50 text-slate-900 text-sm focus:outline-none focus:ring-2 focus:ring-brand-primary focus:border-brand-primary focus:bg-white transition placeholder:text-slate-400 resize-none">{{ old('description', $belt->description) }}</textarea>
                </div>
            </div>
