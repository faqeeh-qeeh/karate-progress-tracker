...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">Tambah Master Sabuk Baru</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">Buat data master sabuk baru untuk pengelompokan tingkatan</p>
        </div>
        <a href="{{ route('admin.belts.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl transition whitespace-nowrap">
        ...
        <form action="{{ route('admin.belts.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input type="color" id="color-picker" value="{{ old('color_code', '#3B82F6') }}" onchange="document.getElementById('color_code').value = this.value" class="w-12 h-11 p-1 rounded-xl border border-slate-300 dark:border-slate-700 cursor-pointer bg-white dark:bg-slate-800">
            ...
            <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                <a href="{{ route('admin.belts.index') }}" class="px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold transition">
                    Batal
                </a>
                <button type="submit" class="px-6 py-2.5 rounded-xl bg-brand-primary hover:bg-brand-primary/90 text-white font-bold text-sm shadow-md transition-all active:scale-[0.99]">
                    Simpan Master Sabuk
                </button>
            </div>
        </form>
