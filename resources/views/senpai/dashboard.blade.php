    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:shadow-md hover:border-brand-primary/40 transition-all duration-200 group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Kohai Terdaftar</p>
                    <p class="text-2xl sm:text-3xl font-black text-brand-primary mt-1">{{ $totalKohai }}</p>
                </div>
            ...
            <div class="flex items-center justify-between text-xs text-slate-500 font-medium mt-3 pt-3 border-t border-slate-100">
                ...
                <a href="{{ route('senpai.kohai.index') }}" class="text-brand-primary font-bold hover:underline">Kelola &rarr;</a>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:shadow-md hover:border-brand-secondary/40 transition-all duration-200 group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Raport Kumite</p>
                    <p class="text-2xl sm:text-3xl font-black text-slate-900 mt-1">{{ $totalKumiteReports }}</p>
                </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:shadow-md hover:border-amber-400 transition-all duration-200 group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Sesi Absensi</p>
                    <p class="text-2xl sm:text-3xl font-black text-slate-900 mt-1">{{ $totalSessions }}</p>
                </div>
                <div class="w-12 h-12 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center text-xl font-bold group-hover:scale-110 transition-transform">
                    📅
                </div>
            </div>
            <div class="flex items-center justify-between text-xs text-slate-500 font-medium mt-3 pt-3 border-t border-slate-100">
                <span>Total Sesi Dibuka</span>
                <span class="font-bold text-slate-700">{{ $activeSession ? '1 Sesi Aktif' : 'Semua Ditutup' }}</span>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs hover:shadow-md hover:border-emerald-400 transition-all duration-200 group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-[11px] font-extrabold text-slate-400 uppercase tracking-wider">Log Kehadiran</p>
                    <p class="text-2xl sm:text-3xl font-black text-emerald-600 mt-1">{{ $totalAttendances }}</p>
                </div>
            ...
            <div class="flex items-center justify-between text-xs text-slate-500 font-medium mt-3 pt-3 border-t border-slate-100">
                ...
                <span class="text-emerald-700 font-extrabold">Terverifikasi</span>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-200/80 shadow-sm p-5 sm:p-6 space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 font-medium mt-0.5">Analisis intensitas serangan (Attack) & persentase akurasi poin pada laga-laga terbaru</p>
                </div>
        ...
        <div class="lg:col-span-1 bg-white rounded-3xl border border-slate-200/80 shadow-sm p-5 sm:p-6 space-y-4 flex flex-col justify-between">
            <div class="border-b border-slate-100 pb-4">
                ...
                <p class="text-xs text-slate-500 font-medium mt-0.5">Distribusi tingkatan sabuk seluruh murid Kohai</p>
            </div>
            ...
            @else
                <div class="h-56 flex flex-col items-center justify-center text-center p-6 text-slate-400">
                    <p class="text-sm font-medium">Belum ada data profil sabuk murid terdata.</p>
                </div>
            @endif

            <div class="pt-3 border-t border-slate-100 grid grid-cols-2 gap-2 text-[11px] font-bold">
                @foreach($beltDistribution as $b)
                    <div class="flex items-center gap-1.5 truncate">
                        <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background-color: {{ $b['color'] }}"></span>
                        <span class="text-slate-600 truncate">{{ $b['name'] }}:</span>
                        <span class="text-slate-900 font-extrabold">{{ $b['count'] }}</span>
                    </div>
                @endforeach
            </div>
        </div>
        ...
        <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-200/80 shadow-sm p-5 sm:p-6 space-y-4">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5 font-medium">Hasil laga WKF yang baru saja dinilai oleh Senpai</p>
                </div>
                <a href="{{ route('senpai.kumite.index') }}" class="inline-flex items-center gap-1 text-xs font-extrabold text-brand-primary hover:text-brand-secondary transition">
                                ...
                                <td class="py-3.5 px-3.5">
                                    <div class="flex items-center gap-1.5 font-bold text-red-600">
                                        ...
                                        @if($r->senshu_corner === 'aka')
                                            <span class="px-1.5 py-0.2 text-[8px] font-black bg-red-100 text-red-700 rounded border border-red-200">SENSHU</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="py-3.5 px-3.5 text-center font-black">
                                    <span class="px-2 py-0.5 rounded-md bg-red-50 text-red-600 border border-red-200/80">{{ $r->aka_total_score }}</span>
                                    <span class="text-slate-300 mx-1">:</span>
                                    <span class="px-2 py-0.5 rounded-md bg-blue-50 text-brand-primary border border-blue-200/80">{{ $r->ao_total_score }}</span>
                                </td>
                                <td class="py-3.5 px-3.5">
                                    <div class="flex items-center gap-1.5 font-bold text-brand-primary">
                                        ...
                                        @if($r->senshu_corner === 'ao')
                                            <span class="px-1.5 py-0.2 text-[8px] font-black bg-blue-100 text-blue-700 rounded border border-blue-200">SENSHU</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="py-3.5 px-3.5 text-center">
                                    @if($r->winner)
                                        <span class="px-2.5 py-1 text-[11px] font-extrabold rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200/80">
                                            🏆 {{ $r->winner->name }}
                                        </span>
                                    @else
                                        <span class="px-2.5 py-1 text-[11px] font-bold rounded-full bg-slate-100 text-slate-500">
                                            Draw / Hantei
                                        </span>
                                    @endif
                                </td>
                ...
                @foreach($schedules as $sched)
                    <div class="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 hover:border-brand-primary/40 transition-all space-y-2">
                        ...
                        <div class="pt-2 border-t border-slate-200/60 flex items-center justify-between text-[11px] text-slate-400 font-medium">
                            ...
                            <span class="text-emerald-600 font-bold">Wajib</span>
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-5 sm:p-6 space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-0.5 font-medium">Monitoring data tingkatan sabuk aktual, riwayat pertarungan, dan presensi latihan</p>
            </div>
            <a href="{{ route('senpai.kohai.index') }}" class="inline-flex items-center gap-1 text-xs font-extrabold text-brand-primary hover:text-brand-secondary transition">
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            <td class="py-3.5 px-4 font-bold text-slate-900">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <a href="{{ route('senpai.kohai.show', $k->id) }}" class="hover:text-brand-primary transition block font-extrabold text-slate-900">
                                            {{ $k->name }}
                                        </a>
                                        <span class="text-[10px] text-slate-400 font-mono">{{ $k->email }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-slate-600">
                                @if($k->kohaiProfile?->isPolindra())
                                    <span class="font-bold text-slate-800 block">{{ $k->kohaiProfile->nim ?? '-' }}</span>
                                    <span class="text-[10px] text-slate-400 truncate">{{ $k->kohaiProfile->studyProgram->name ?? 'Polindra' }}</span>
                                @else
                                    <span class="font-bold text-slate-800 block">{{ $k->kohaiProfile->institution ?? 'Non-Polindra' }}</span>
                                    <span class="text-[10px] text-slate-400">Umum / Luar</span>
                                @endif
                            </td>

                            <td class="py-3.5 px-4">
                                <div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 border border-slate-200/80 text-[11px] font-extrabold text-slate-800">
                            ...
                            <td class="py-3.5 px-4 text-center font-bold text-slate-800">
                                <span class="px-2.5 py-1 rounded-lg bg-blue-50 text-brand-primary border border-blue-200/60 font-black">
                                    {{ $totalMatches }} Laga
                                </span>
                            </td>

                            <td class="py-3.5 px-4 text-center">
                                <span class="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200/60 font-black">
